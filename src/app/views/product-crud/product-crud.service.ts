...
import { catchError, map, delay } from '../../../../node_modules/rxjs/operators';
...
export class ProductCrudService {
  ...
  updateProduct(id, item, items) {
    ...
    return this.http.put(this.productApiUrl + id, item, this.httpOptions).
      pipe(
        map(data => {
          items = items.map(i => {
            if (i.id === id) {
  ...
  addProduct(productObj, items): Observable<any> {
    return this.http.post(this.productApiUrl, productObj, this.httpOptions).
      pipe(
        map(data => {
