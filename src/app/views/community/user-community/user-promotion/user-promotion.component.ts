...
export class UserPromotionComponent implements OnInit {
  ...
  createPromotion(res, usage, tempQuoata) {
    this.userPromotionService.createPromotion(res).subscribe(
      response => {
        const temData: any = response;
        ...
        if (usage < tempQuoata - 1 && tempQuoata - usage === 2) {
          this.appWarningService.showWarning({
            title: "License",
            message:
              "Your subscription plan is about to expire!</br>One more promotion remaining!"
          });
    ...
    this.confirmService
      ...
      .subscribe(res => {
        if (res) {
          ...
          this.userPromotionService.deletePromotionList(idArray).subscribe(
            response => {
              this.loader.close();
              this.selectAll = false;
              this.indeterminateState = false;
    ...
    this.confirmService
      ...
      .subscribe(res => {
        if (res) {
          ...
          this.userPromotionService.deletePromotionById(data.id).subscribe(
            response => {
              this.comunityService
                .licenseExpireState(userObj.userData.client.id, "promos")
                .subscribe(resData => {
